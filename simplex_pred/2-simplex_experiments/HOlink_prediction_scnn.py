...
def main():
    ...
    parser.add_argument("-m","--model_name",help="the model name", type=str, default='scnn')
    parser.add_argument("-l", "--layers", help="the number of layers", type=int, default=2) # note that here it is the number of intermediate layers
    ...
    '#edges of val positive triangle:', len(val_pos_i), len(val_pos_j), len(val_pos_k), '\n','#edges of val negative triangle:', len(val_neg_i), len(val_neg_j), len(val_neg_k), '\n',
    ...
    if model_name == 'snn':
        model_save_path = model_path+'/%dlayers_%dorders_%dfeatures_%drlz'%(args.layers,K,hidden_features,args.realizations)     
         ...
         model_save_path = model_path+'/%dlayers_%d_%dorders_%dfeatures_%drlz'%(args.layers,K1,K2,hidden_features,args.realizations)
    elif model_name == 'psnn':
