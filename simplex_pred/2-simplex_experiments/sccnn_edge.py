...
import time 
'''sccnn-edge implementation'''
from sccnn_model import sccnn

'''to implement the sc-cf, one needs to import sccnn from sccnn_filter_model'''
...
def main():
    ...
    parser.add_argument("-m","--model_name",help="the model name", type=str, default='sccnn_edge')
    ...
    if model_name in ['sccnn_edge','sccnn_edge_ebli']:
        losslogf = open("./%s_%dlayers_%dorder_%dfeatures_%drlz.txt" %(model_name,args.layers,k,hidden_features,args.realizations),"w")
        time_measure = open("./time_%s_%dlayers_%dorder_%dfeatures_%drlz.txt"%(model_name,args.layers,k,hidden_features,args.realizations),"w")
