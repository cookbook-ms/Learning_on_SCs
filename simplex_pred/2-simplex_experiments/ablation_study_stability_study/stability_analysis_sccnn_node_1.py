...
def main():
    ...
    num_perts = 10
    pert_list_1 = np.linspace(0,1,num_perts) # perturbation on L1l   
    # pert_list_1 = np.logspace(-3,0,num_perts) 
    num_rlz = 10
    ...
    epsilon_rlt_1u = np.empty((len(pert_list_1),len(rlz_idx)))
    epsilon_rlt_2 = np.empty((len(pert_list_1),len(rlz_idx))) 
    epsilon_01 = np.empty((len(pert_list_1),len(rlz_idx))) 
    epsilon_12 = np.empty((len(pert_list_1),len(rlz_idx))) 
    ...
    for j in np.arange(len(rlz_idx)):
        ...
        for i in range(len(pert_list_1)):
            epsilon_1 = pert_list_1[i] 
            pert_edge = np.mean([np.diag(np.random.rand(L1.shape[0])*epsilon_1-0.5*epsilon_1) for k in range(10)],0) 
            if epsilon_1 == 0:
                pert_edge = np.zeros_like(pert_edge)
            '''L1 pert'''
            D2 = np.maximum(np.diag(abs(b2_train)@np.ones(b2_train.shape[1])),np.identity(b2_train.shape[0]))
            D2_pert = D2 + pert_edge
            # print(np.diag(D2),np.diag(D2_pert))
            
            D1 = 2 * np.diag(abs(b1)@D2@np.ones(D2.shape[0]))
            D3 = 1/3*np.identity(b2_train.shape[1])
            L1l_pert = sla.fractional_matrix_power(D2_pert,0.5) @ b1.T @ la.pinv(D1) @ b1 @ sla.fractional_matrix_power(D2_pert,0.5)
            L1u_pert = la.inv(sla.fractional_matrix_power(D2_pert,0.5)) @ b2_train @ D3 @ b2_train.T @ la.inv(sla.fractional_matrix_power(D2_pert,0.5))
 
            '''L0 pert'''
            b1_train = b1
            D20 = np.maximum(np.diag(abs(b1_train)@np.ones(b1_train.shape[1])),np.identity(b1_train.shape[0]))
            L0_pert =  la.inv(sla.fractional_matrix_power(D20,0.5)) @ b1_train @ (np.eye(len(f_edge))+pert_edge) @ b1_train.T @ la.inv(sla.fractional_matrix_power(D20,0.5)) 
            '''L2 pert'''
            D5 = np.diag(abs(b2_train)@np.ones(b2_train.shape[1])) 
            D5_pert = D5 + pert_edge 
            L2_pert = b2_train.T @ la.pinv(D5_pert) @ b2_train
            # print(np.diag(D5),np.diag(D5_pert))

            pert_edge = torch.tensor(pert_edge,dtype=torch.float32,device=device)
            ...
            D1 = torch.tensor(D1,dtype=torch.float32,device=device)   
            D2 = torch.tensor(D2,dtype=torch.float32,device=device)    
            D2_pert = torch.tensor(D2_pert,dtype=torch.float32,device=device)    
            D3 = torch.tensor(D3,dtype=torch.float32,device=device)    
            D5 = torch.tensor(D5,dtype=torch.float32,device=device)
            D5_pert = torch.tensor(D5_pert,dtype=torch.float32,device=device)
 

            '''measure the influence of pert edge on three laplacians and projection matrices '''
            epsilon_rlt_0[i,j] = tla.norm(L0_pert-L0,2)/tla.norm(L0,2)
            epsilon_rlt_1l[i,j] = tla.norm(L1l_pert-L1l,2)/tla.norm(L1l,2)
            epsilon_rlt_1u[i,j] = tla.norm(L1u_pert-L1u,2)/tla.norm(L1u,2)
            epsilon_rlt_2[i,j] = tla.norm(L2_pert-L2,2)/tla.norm(L2,2)
            print('pert on laps',epsilon_1,epsilon_rlt_0[i,j],epsilon_rlt_1l[i,j],epsilon_rlt_1u[i,j],epsilon_rlt_2[i,j])

            proj_01 = D2 @ B1.T @ torch.inverse(D1)
            proj_01_pert = D2_pert @ B1.T @ torch.inverse(D1)
            proj_12 = B2_train.T @ torch.pinverse(D5) 
            proj_12_pert = B2_train.T @ torch.pinverse(D5_pert)
            epsilon_01[i,j] = tla.norm(proj_01_pert - proj_01,2)/tla.norm(proj_01,2)
            epsilon_12[i,j] = tla.norm(proj_12_pert - proj_12,2)/tla.norm(proj_12,2) 
            print('pert on projs',epsilon_01[i,j], epsilon_12[i,j])

            model_pert = sccnn(F_in=1, F_intermediate=F_intermediate, F_out=hidden_features, b1=B1, b2=B2_train, l0=L0_pert, l1l=L1l_pert, l1u=L1u_pert, l2=L2_pert, d1=D1, d2=D2_pert, d3=D3, d5=D5_pert,k00=k00,k0p=k0p,k1n=k1n,k11=k11,k12=k12,k1p=k1p,k2n=k2n,k22=k22, sigma=sigma, model_name=model_name)
    ...
    var_path = r'./pert_var_nn_' + model_name + '_pert_1'
