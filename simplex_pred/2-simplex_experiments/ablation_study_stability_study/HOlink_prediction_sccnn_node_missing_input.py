...
'''ablation study of sccnn-node with limited input'''
...
def main():
    ...
    parser.add_argument("-m","--model_name",help="the model name", type=str, default='sccnn_node_missing_edge')
    parser.add_argument('-r',"--missing_rate",help="missing rate in the data",type=float,default=0.5) # default no missing input data
    ...
    if model_name == 'sccnn_node_zero_edge':
        f_edge = torch.tensor(np.zeros(b1.shape[1]),dtype=torch.float32)
        f_node = torch.tensor(np.array(cochains[0]),dtype=torch.float32)
    elif model_name == 'sccnn_node_missing_edge':
        f_node = torch.tensor(np.array(cochains[0]),dtype=torch.float32)
        f_edge = torch.tensor(np.array(cochains[1]),dtype=torch.float32) 
        zero_idx = np.random.permutation(b1.shape[1])
        f_edge[zero_idx[0:math.floor(args.missing_rate*b1.shape[1])]] = 0  
    elif model_name == 'sccnn_node_zero_node':
        f_edge = torch.tensor(np.array(cochains[1]),dtype=torch.float32)
        f_node = torch.tensor(np.zeros(b1.shape[0]),dtype=torch.float32)    
    elif model_name == 'sccnn_node_missing_node':
        f_edge = torch.tensor(np.array(cochains[1]),dtype=torch.float32)   
        f_node = torch.tensor(np.array(cochains[0]),dtype=torch.float32) 
        zero_idx = np.random.permutation(b1.shape[0])
        f_node[zero_idx[0:math.floor(args.missing_rate*b1.shape[0])]] = 0  
    elif model_name == 'sccnn_node_zero_all_input':
        f_node = torch.tensor(np.zeros(b1.shape[0]),dtype=torch.float32)
        f_edge = torch.tensor(np.zeros(b1.shape[1]),dtype=torch.float32)  
    elif model_name == 'sccnn_node_missing_node_edge':
        f_edge = torch.tensor(np.array(cochains[1]),dtype=torch.float32) 
        zero_idx = np.random.permutation(b1.shape[1])
        f_edge[zero_idx[0:math.floor(args.missing_rate*b1.shape[1])]] = 0        
        f_node = torch.tensor(np.array(cochains[0]),dtype=torch.float32) 
    ...
    if model_name in ['sccnn_node_zero_node','sccnn_node_zero_edge','sccnn_node_zero_all_input','sccnn_node_missing_node','sccnn_node_missing_edge','sccnn_node_missing_node_edge']:
        if model_name in ['sccnn_node_missing_node','sccnn_node_missing_edge','sccnn_node_missing_node_edge']:
            losslogf = open("./%s%g_%dlayers_%dorder_%dfeatures_%drlz.txt" %(model_name,args.missing_rate,args.layers,k,hidden_features,args.realizations),"w")
        else:
    ...
    if model_name in ['sccnn_node_zero_node','sccnn_node_zero_edge','sccnn_node_zero_all_input','sccnn_node_missing_node','sccnn_node_missing_edge','sccnn_node_missing_node_edge']:
        if model_name in ['sccnn_node_missing_node','sccnn_node_missing_edge']:
            model_save_path = model_path+'/%g_%dlayers_%dorder_%dfeatures_%drlz'%(args.missing_rate,args.layers,k,hidden_features,args.realizations)
        else:
